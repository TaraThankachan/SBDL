...
if __name__ == '__main__':
    ...
    job_run_env = sys.argv[1].upper() # QA
    load_date = sys.argv[2] #load date
    job_run_id = "SBDL-" + str(uuid.uuid4()) #SBDL-UUID

    print("Initializing SBDL Job in " + job_run_env + " Job ID: " + job_run_id) # prnt statement
    conf = ConfigLoader.get_config(job_run_env)#
    enable_hive = True if conf["enable.hive"] == "true" else False #boolean
    hive_db = conf["hive.database"] #hive.database = sbdl_db_qa

    logger.info("Creating Spark Session")
    spark = Utils.get_spark_session(job_run_env)# creating spark session in QA
    logger = Log4j(spark)
